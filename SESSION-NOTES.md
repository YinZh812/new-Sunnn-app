...
`index.html` 从 2109 行单文件起步 → 现在 948 行。模块化源码（`src/` 33 个文件）已完整接管 Hero、列表、分析页、设置页、目标页、推荐词、7 个 modal、导航系统、渲染枢纽、**输入流**。inline `<script>` 剩余约 600 行：内联编辑、WheelTime、类别设置、手动记账变量/函数、语音识别、高级主题、列表滑删。
